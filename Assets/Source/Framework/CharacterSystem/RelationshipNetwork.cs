...
namespace CharacterSystem
{
    ...
    public class RelationshipNetwork : MonoBehaviour, ICharacterSubsystem
    {
        ...
        /// <summary>
        /// Gets the relationship value for a specific parameter between two entities
        /// </summary>
        public float GetRelationshipValue(string entityId1, string entityId2, RelationshipParameter parameter)
        {
            // Get relationship data
            RelationshipData relationship = GetRelationshipStatus(entityId1, entityId2);
            ...
            switch (parameter)
            {
                case RelationshipParameter.Friendship:
                    return relationship.strength;
                case RelationshipParameter.Trust:
                    return relationship.trust;
                case RelationshipParameter.Familiarity:
                    return relationship.familiarity;
                case RelationshipParameter.Romance:
                    // In a real implementation, this would come from a specific attribute
                    // For now, just use a fraction of the strength
                    return relationship.strength * 0.8f;
                case RelationshipParameter.Respect:
                    // In a real implementation, this would be separate
                    // For now, just use a combination of trust and familiarity
                    return (relationship.trust + relationship.familiarity) / 2f;
                case RelationshipParameter.Comfort:
                    // In a real implementation, this would be separate
                    // For now, just use a weighted average of trust and familiarity
                    return (relationship.trust * 0.7f + relationship.familiarity * 0.3f);
                default:
                    return 0f;
            }
        }
        ...
        /// <summary>
        /// Sets the relationship value for a specific parameter between two entities
        /// </summary>
        public void SetRelationshipValue(string entityId1, string entityId2, RelationshipParameter parameter, float value)
        {
            // Get the relationship (or create it if it doesn't exist)
            RelationshipData relationship = _relationshipGraph.relationships.FirstOrDefault(
                r => (r.sourceId == entityId1 && r.targetId == entityId2) || 
                     (r.sourceId == entityId2 && r.targetId == entityId1));
            ...
            if (relationship == null)
            {
                relationship = CreateRelationship(entityId1, entityId2);
                _relationshipGraph.relationships.Add(relationship);
            }
            
            // Ensure the relationship is oriented correctly
            if (relationship.sourceId != entityId1)
            {
                // Create a flipped copy and add it
                RelationshipData flippedRelationship = new RelationshipData
                {
                    sourceId = entityId1,
                    targetId = entityId2,
                    type = relationship.type,
                    ...
                    trust = relationship.trust,
                    familiarity = relationship.familiarity,
                    history = new List<RelationshipHistoryEntry>(relationship.history),
                    hiddenAttributes = new Dictionary<string, object>(relationship.hiddenAttributes)
                };
                
                relationship = flippedRelationship;
                _relationshipGraph.relationships.Add(relationship);
            }
            ...
            switch (parameter)
            {
                ...
                case RelationshipParameter.Trust:
                    ...
                    break;
                case RelationshipParameter.Familiarity:
                    relationship.familiarity = value;
                    break;
                case RelationshipParameter.Romance:
                case RelationshipParameter.Respect:
                case RelationshipParameter.Comfort:
                    // In a real implementation, these would be stored separately
                    // For now, add them as hidden attributes
                    relationship.hiddenAttributes[parameter.ToString()] = value;
                    break;
            }
        }
        
        /// <summary>
        /// Gets the average relationship value between two entities
        /// </summary>
        public float GetAverageRelationship(string entityId1, string entityId2)
        {
            // Get relationship data
            RelationshipData relationship = GetRelationshipStatus(entityId1, entityId2);
            ...
            // Calculate average of relevant parameters
            float sum = 0f;
            int count = 0;
            ...
            sum += relationship.trust;
            count++;
            
            sum += relationship.familiarity;
            count++;
        ...
        /// </summary>
        public List<RelationshipData> GetAllRelationships(string entityId)
        {
            return _relationshipGraph.relationships
    ...
    public interface IRelationshipSystem : ICharacterSubsystem
    {
        ...
        // Get average relationship between two characters
        float GetAverageRelationship(string entityId1, string entityId2);
    }
}